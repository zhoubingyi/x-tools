(() => {
  if (window.__xToolsContentInstalled) return;
  window.__xToolsContentInstalled = true;
  ...
  function readSettings() {
    chrome.storage.sync.get({ [STORAGE_KEY]: DEFAULTS }, (items) => {
      settings = { ...DEFAULTS, ...(items[STORAGE_KEY] || {}) };
      scheduleRender();
    });
  }
  ...
  function renderBadge(article, data) {
    ...
    // Insert before the caret (three-dot menu) so badge sits between user-name and menu
  ...
  async function downloadTweetMedia(btn, data) {
    ...
    try {
      ...
      for (const task of tasks) {
        const result = await sendDownload(task.url, task.filename);
  ...
  function sendDownload(url, filename) {
    return new Promise((resolve) => {
      chrome.runtime.sendMessage({ type: 'XT_DOWNLOAD', url, filename }, (response) => {
  ...
  function getDownloadHistory() {
    return new Promise((resolve) => {
      chrome.storage.local.get({ [DOWNLOAD_HISTORY_KEY]: [] }, (items) => {
        resolve(Array.isArray(items[DOWNLOAD_HISTORY_KEY]) ? items[DOWNLOAD_HISTORY_KEY] : []);
      });
    });
  }
  ...
  function setDownloadHistory(history) {
    return new Promise((resolve) => {
      chrome.storage.local.set({ [DOWNLOAD_HISTORY_KEY]: history }, resolve);
    });
  }
  ...
  function createDashboard() {
    ...
    dashboardEl.innerHTML = `
      <div class="xt-dashboard-header">
        ...
        <button class="xt-dashboard-close" type="button" title="关闭" aria-label="关闭">
          ×
        </button>
      </div>
    ...
    dashboardEl.querySelectorAll('.xt-dashboard-tab-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        dashboardEl.querySelectorAll('.xt-dashboard-tab-btn').forEach(b => 
          b.classList.remove('active'));
        btn.classList.add('active');
        
        // Update active panel
        dashboardEl.querySelectorAll('.xt-dashboard-panel').forEach(p => 
  ...
  // Create a toggle button to open the dashboard
  function createDashboardToggleButton() {
    ...
    // Position the button in the top-right corner of the viewport
    toggleBtn.style.position = 'fixed';
    toggleBtn.style.top = '20px';
    toggleBtn.style.right = '20px';
    toggleBtn.style.zIndex = '999999';
    toggleBtn.style.padding = '8px 12px';
    toggleBtn.style.backgroundColor = '#bf5a2a';
  ...
  // Initialize settings and create toggle button
  readSettings();
